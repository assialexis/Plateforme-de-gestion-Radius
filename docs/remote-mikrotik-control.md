...
│
├─ 1. Résoudre admin_id (session → voucher → admin connecté)
│
├─ 2. Trouver le NAS de l'admin (findNasForAdmin)
│     ├─ Requête DB filtrée par admin_id
│     ├─ Priorité : match IP exact > NAS avec router_id actif > fallback
│     └─ Garantit le ciblage du bon routeur en multi-tenant
│
├─ 3. Canal API directe : tryDirectApiDisconnect()
...
│
├─ 4. Canal Pull-Based : commandSender->disconnectHotspotUser()
│     └─ INSERT INTO router_commands (type=disconnect_hotspot, priority=10)
│
└─ 5. Marquage BDD : UPDATE sessions SET stop_time=NOW()
```
...
### Résolution de l'admin_id

Pour garantir que la commande arrive au bon routeur, le système résout l'`admin_id` en cascade :

1. **`session.admin_id`** : directement depuis la session (rempli lors de la création)
2. **`voucher.admin_id`** : si la session n'a pas d'admin_id, on le récupère depuis le voucher associé
3. **`getAdminId()`** : dernier recours, utilise l'ID de l'administrateur connecté

Si l'admin_id était manquant, il est automatiquement mis à jour sur la session pour les prochaines fois.

### Déconnexion en masse (Disconnect All)

Même stratégie mais optimisée : les sessions sont groupées par `admin_id` pour cibler le bon routeur de chaque admin.

```
SessionController::disconnectAll()
│
├─ Grouper les sessions par admin_id
│
└─ Pour chaque admin :
     ├─ Trouver le NAS de l'admin (findNasForAdmin)
...
Quand une action est déclenchée depuis une session, le système doit identifier quel routeur NAS est concerné. La méthode `findNasForAdmin()` utilise une résolution **basée sur l'admin_id** en 3 niveaux :

```
Session (admin_id résolu)
         │
    ┌────┴──────────────────────────────────────┐
    │  Priorité 1 : Match IP exact              │
    │  NAS de l'admin où nasname = session IP    │──→ Match trouvé
    │  OU mikrotik_host = session IP             │
    └────┬──────────────────────────────────────┘
         │ Non trouvé
    ┌────┴──────────────────────────────────────┐
    │  Priorité 2 : NAS avec router_id actif    │
    │  NAS de l'admin avec router_id non-vide   │──→ Match trouvé
    │  Préfère le plus récemment vu (last_seen) │
    └────┬──────────────────────────────────────┘
         │ Non trouvé
    ┌────┴──────────────────────────────────────┐
    │  Priorité 3 : N'importe quel NAS          │
    │  de cet admin (dernier recours)           │──→ Fallback
    └───────────────────────────────────────────┘
```

### Architecture décentralisée (noeud RADIUS)

Dans l'architecture avec des noeuds RADIUS décentralisés, les sessions sont créées sur le noeud puis synchronisées vers le serveur central. L'`nas_ip` dans la session correspond à l'IP du MikroTik **vue par le noeud RADIUS** (IP locale), qui ne correspond pas aux NAS configurés sur le serveur central (souvent en wildcard `0.0.0.0/0`).

C'est pourquoi la résolution est **basée sur l'admin_id** plutôt que sur le match IP :

```
Noeud RADIUS                    Serveur Central
┌────────────────┐              ┌────────────────────────┐
│ MikroTik       │              │ Table NAS              │
│ IP: 192.168.1.1│              │ ┌────────────────────┐ │
│    │           │              │ │ Admin 1 (superuser)│ │
│    ▼           │              │ │ nasname: 0.0.0.0/0 │ │
│ Session créée  │   sync       │ │ router_id: NAS-A1  │ │
│ nas_ip:        │──────────→   │ ├────────────────────┤ │
│ 192.168.1.1    │              │ │ Admin 5            │ │
│                │              │ │ nasname: 0.0.0.0/0 │ │
│                │              │ │ router_id: NAS-B2  │ │
...
Sans filtre admin_id → le premier wildcard (Admin 1) est pris = MAUVAIS
Avec filtre admin_id → seul le NAS de l'Admin 5 est considéré = CORRECT
```

### Pourquoi c'est important

En multi-tenant, chaque administrateur a ses propres NAS (souvent en wildcard `0.0.0.0/0`). Sans filtrage par `admin_id`, le système pourrait envoyer la commande de déconnexion au routeur du superuser au lieu de celui de l'admin concerné.

Le système garantit le bon ciblage en :
1. Résolvant l'`admin_id` depuis la session, le voucher, ou l'admin connecté
2. Filtrant strictement les NAS par cet `admin_id`
3. Préférant le NAS avec un `router_id` actif (le routeur qui poll le serveur)
...
| `admin_id` | Propriétaire du NAS (multi-tenant) | `5` |

**Recommandation** : toujours renseigner `mikrotik_host` même si le NAS est en wildcard, pour que l'API directe fonctionne. L'`admin_id` est obligatoire pour le bon routage des commandes.
...
Le système supporte deux architectures de synchronisation des sessions :

#### Mode direct (MikroTik → Serveur central)

Le script `nas-sync` sur le MikroTik s'exécute toutes les **5 minutes** et envoie au serveur :
...
#### Mode noeud RADIUS décentralisé (MikroTik → Noeud → Serveur central)

Dans cette architecture, un noeud RADIUS local gère l'authentification et le comptage :

```
MikroTik ──RADIUS──→ Noeud RADIUS ──sync_client.php──→ Serveur Central
                     (FreeRADIUS)     (cron 1min)       (importNodeSyncData)
```

- Le MikroTik communique avec le noeud RADIUS local (FreeRADIUS)
- Le noeud stocke les sessions dans sa BDD locale (`radius_node`)
- `sync_client.php` (cron toutes les minutes) pousse les sessions vers le serveur central
- Le serveur central importe les sessions via `importNodeSyncData()` avec l'`admin_id` du voucher
- Les sessions actives (`stop_time IS NULL`) sont re-poussées à chaque sync pour mettre à jour les compteurs

**Important** : Dans ce mode, l'`nas_ip` de la session correspond à l'IP du MikroTik vue par le noeud (IP locale), pas par le serveur central. C'est pourquoi la résolution du NAS pour les commandes se fait par `admin_id` et non par IP.
...
5. **Fermer les sessions orphelines** : sessions sans mise à jour depuis 5 minutes (perte de connexion)
...
1. **Vérifiez l'admin_id** : Le système utilise l'`admin_id` pour trouver le bon NAS
   - Vérifiez que la session a un `admin_id` : `SELECT id, admin_id FROM sessions WHERE stop_time IS NULL`
   - Vérifiez que le voucher a un `admin_id` : `SELECT id, admin_id FROM vouchers WHERE username = 'XXX'`
   - Vérifiez que le NAS de l'admin a un `router_id` : `SELECT id, admin_id, router_id, last_seen FROM nas WHERE admin_id = X`
   - Consultez les logs PHP pour le message `Disconnect session #...` qui montre l'admin_id résolu et le NAS sélectionné
...
| `src/Api/SessionController.php` | Déconnexion hotspot (dual canal, résolution admin_id + findNasForAdmin) |
...
| `radius-node/sync_client.php` | Client de sync noeud RADIUS → central (cron 1min) |
| `web/node_sync.php` | Endpoint central pour recevoir les données du noeud |
