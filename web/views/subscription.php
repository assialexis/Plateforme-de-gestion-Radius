...
<div x-data="subscriptionPage()" x-init="init()">
    ...
    <div x-show="!loading" x-cloak>
        ...
        <template x-if="pendingList.length > 0">
            <div class="mb-6 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-2xl overflow-hidden">
                <div class="px-5 py-3 border-b border-amber-200/50 dark:border-amber-800/20 flex items-center gap-2">
                    ...
                    <h3 class="text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider"><?= __('credits.pending_recharges_title') ?? 'Recharges en attente' ?></h3>
                </div>
                <div class="divide-y divide-amber-200/30 dark:divide-amber-800/20">
                    <template x-for="ptx in pendingList" :key="ptx.transaction_id">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-amber-800 dark:text-amber-300">
                                        <span x-text="parseFloat(ptx.amount).toLocaleString()"></span> <span x-text="ptx.currency"></span>
                                        <span class="text-xs font-normal text-amber-600 dark:text-amber-400 ml-1" x-text="'via ' + ptx.gateway_code"></span>
                                    </p>
                                    <p class="text-[11px] text-amber-600/70 dark:text-amber-500" x-text="formatTxDate(ptx.created_at)"></p>
                                </div>
                            </div>
                            <button @click="verifyPendingRecharge(ptx.transaction_id)"
                                :disabled="verifyingTxn === ptx.transaction_id"
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-amber-600 hover:bg-amber-700 text-white transition-colors shadow-sm disabled:opacity-50">
                                <svg x-show="verifyingTxn !== ptx.transaction_id" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                <svg x-show="verifyingTxn === ptx.transaction_id" class="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                <span x-text="verifyingTxn === ptx.transaction_id ? '<?= __js('credits.verifying') ?>' : '<?= __js('credits.verify_payment') ?>'"></span>
                            </button>
                        </div>
                    </template>
                </div>
...
function subscriptionPage() {
    return {
        ...
        async loadBalance() {
            try {
                ...
                if (data.success) {
                    ...
                    this.pendingList = data.data.pending_list || [];
        ...
        async verifyPendingRecharge(txnId) {
            this.verifyingTxn = txnId;
            try {
                const res = await fetch('api.php?route=/credits/recharge/verify', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
                ...
                if (data.success && data.data) {
                    if (data.data.status === 'completed') {
                        this.notify('<?= __js('credits.recharge_success') ?>', 'success');
                        await Promise.all([this.loadBalance(), this.loadTransactions()]);
                    } else {
                        this.notify('<?= __js('credits.recharge_still_pending') ?>', 'info');
                    }
                } else {
                    this.notify(data.message || 'Erreur', 'error');
                }
