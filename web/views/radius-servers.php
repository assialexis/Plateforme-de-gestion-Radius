            ...
            <div class="group relative bg-white dark:bg-[#161b22] rounded-2xl shadow-sm hover:shadow-xl dark:shadow-none border border-gray-200/60 dark:border-[#30363d] overflow-hidden transition-all duration-300 transform hover:-translate-y-1">
                ...
                <div class="p-6">
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span x-show="server.is_default == 1" class="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                                        <?= __('radius_servers.default_badge') ?>
                                    </span>
                                </div>
                    ...
                    <div class="flex items-center gap-2 pt-3 border-t border-gray-100 dark:border-[#30363d]">
                        ...
                        <button @click="setDefault(server)" class="px-3 py-2 text-sm font-medium rounded-lg transition-colors"
                            :class="server.is_default == 1 ? 'text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30' : 'text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/20 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400'"
                            :title="server.is_default == 1 ? '<?= __js('radius_servers.remove_default') ?>' : '<?= __js('radius_servers.set_default') ?>'">
...
function radiusServersPage() {
    return {
        ...
        async setDefault(server) {
            try {
                const res = await fetch(`api.php?route=/radius-servers/${server.id}/set-default`, { method: 'POST' });
                const json = await res.json();
                if (json.success) {
                    this.notify(json.message, 'success');
                    await this.loadServers();
